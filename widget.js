...
cpdefine("inline:com-chilipeppr-widget-autolevel", ["chilipeppr_ready", "ThreeHelvetiker", "Three"], function () {
    ...
    return {
        ...
        maxx: null,
        maxy: null,
        absOffset : null, //offset to absolute coordinates, used to set the first probe position to zero and correct all subsequent positions
            ...
            //setTimeout(function() {
                that.watchZ(probe, function() {
                    ...
                    if (that.currentStep == 0) {
                        ...
                        console.log("got done watchz callback on 1st step. zeroing out z.");
                        //that.send("G28.3 Z0\n");
                        that.send("G10 L2 P1 Z" + that.absOffset +"\n"); // set the first probe value as offset for G54
                    }
                    
                    setTimeout(that.doNextStep.bind(that), 20);
                });
                ...
                */
                ...
                if ('r' in json && 'prb' in json.r) {
                    ...
                    if (this.currentStep == 0) {
                        ...
                        //probe.z = 0;
                        //this.send("G28.3 Z0\n");
                        this.status("Zeroing out Z on 1st probe as baseline Z.");
                        that.absOffset = probe.z;
                    }
                    var pos = '{x:' + probe.x + ',y:' + probe.y + '}';
                    //this.status("Working on probe for " + pos + " Found lowest Z:" + probe.z);
                    probe.z = probe.z - that.absOffset;
                    this.status("Working on probe for " + pos + " Found lowest Z:" + probe.z);
